-- ============================================================
-- 0005_submit_quiz.sql
-- p_reponses : jsonb array, ex.
-- [
--   { "question_id": "...", "type": "qcm", "reponse": "option-uuid" },
--   { "question_id": "...", "type": "choix_multiple", "reponse": ["opt1","opt2"] },
--   { "question_id": "...", "type": "texte_libre", "reponse": "Paris" },
--   { "question_id": "...", "type": "ordonnancement", "reponse": ["opt3","opt1","opt2"] },
--   { "question_id": "...", "type": "association", "reponse": {"gauche1": "droite2"} }
-- ]
-- ============================================================
...
begin
  ...
  select max(attempt_number) into v_tentative_precedente
  ...
  -- Parcourt chaque réponse soumise et la corrige selon le type de la question
  ...
  loop
    ...
    if v_question.id is null then
      continue; -- question inconnue, ignorée silencieusement
    end if;
    ...
    case v_question.type
      ...
      when 'code' then
        -- Correction automatique non couverte (nécessite un exécuteur sandboxé)
        v_is_correct := null;
  ...
  -- Si réussi, on déclenche la même mécanique de complétion que les autres leçons
  -- (idempotent : ne re-crédite pas l'XP si déjà validée précédemment)
  if v_reussi then
