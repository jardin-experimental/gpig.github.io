-- ============================================================
-- 0011_fix_qcm_array_literal.sql
-- Pour les questions qcm/vrai_faux/image, on ne sélectionnait qu'un seul
-- id (limit 1) mais dans une variable déclarée comme tableau (uuid[]) —
-- Postgres tente alors de parser la chaîne UUID comme un littéral de
-- tableau ('{...}') et échoue avec "malformed array literal". Corrigé en
-- utilisant une variable scalaire dédiée pour ce cas.
-- ============================================================
  ...
  loop
    ...
    case v_question.type
      when 'qcm', 'vrai_faux', 'image' then
        ...
        v_is_correct := (v_reponse->>'reponse')::uuid = v_correct_option_id;
